...
def sefaresh(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            sabt = True
            return render(request, 'sefaresh_hoda.html' , {'sabt' :sabt})
    ...
    return render(request,'sefaresh_hoda.html',{'form':form})
...
def endbuy(request , username , id_book):
    ...
    for id in book_list:
        newbook = []
        #return render_to_response('hoda.html', {'id': id})
...
def Rateing(request , username , id_book):
    new_book = Book.objects.get(id_book=id_book)
    #new_book = Book.objects.get(id_book__icontains=id_book)
...
def button(request):
    if request.POST.get('sabt'):
        x=24
    return render_to_response('tamrini.html', {'x':x})
